...
const Bet = ({ index, add, setAdd }: BetProps) => {
	...
	const [showCashoutAnim, setShowCashoutAnim] = useState(false);
	const [cashoutWinAmount, setCashoutWinAmount] = useState(0);
	...
	const minus = (type: FieldNameType) => {
		...
		if (type === "betAmount") {
			if (betAmount - 10 < minBet) {
				...
				value.userInfo[index][type] = Number((Number(betAmount) - 10).toFixed(2))
			}
		} else {
	...
	const plus = (type: FieldNameType) => {
		let value = state;
		if (type === "betAmount") {
			if (value.userInfo[index][type] + 10 > state.userInfo.balance) {
				value.userInfo[index][type] = Math.round(state.userInfo.balance * 100) / 100
			} else {
				if (value.userInfo[index][type] + 10 > maxBet) {
	...
	return (
		<div className="bet-control">
									...
									{betState || betted ?
										...
										<input type="number" value={Number(myBetAmount)}
											onChange={e => {
												...
												setMyBetAmount(newAmount);
												let attrs = state;
												attrs.userInfo[index].betAmount = newAmount;
												update(attrs);
											}}></input>
									}
								</div>
					...
					<div className="buttons-block">
						{betted ? GameState === "PLAYING" ?
							<button className="btn-waiting" onClick={() => {
								setCashoutWinAmount(Number((betAmount * currentTarget).toFixed(2)));
								setCashoutMultiplier(Number(currentTarget.toFixed(2)));
			...
			{/* Cashout Celebration Animation */}
			{showCashoutAnim && (
				<div className="cashout-celebration">
					<div className="cashout-celebration-content">
						<div className="cashout-win-text">
							<div className="cashout-label">YOU WON!</div>
							<div className="cashout-amount">₹{cashoutWinAmount.toFixed(2)}</div>
							<div className="cashout-multiplier">{cashoutMultiplier.toFixed(2)}x</div>
						</div>
					</div>
				</div>
			)}
